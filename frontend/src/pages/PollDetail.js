...
const PollDetail = () => {
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      try { // Update localStorage
        const alreadyVotedPolls = JSON.parse(localStorage.getItem('alreadyVotedPolls') || '{}');
        alreadyVotedPolls[id] = true;
        localStorage.setItem('alreadyVotedPolls', JSON.stringify(alreadyVotedPolls));
      } catch (e) { console.error("LocalStorage write error post-vote:", e); }
      setVoteSuccess(true);
      if (poll.onChain?.hasRewards) { setRewardReceived(true); }
      setTimeout(async () => { // Refresh data
        try {
            const response = await getPoll(id);
  ...
  // Determine if user has voted (run once)
  ...
  return (
    <div className="max-w-3xl mx-auto my-10 p-6 md:p-8 bg-white rounded-xl shadow-lg border border-gray-200/75">
      ...
      <div className="mb-6 pb-4 border-b border-gray-200">
        {/* ... (rest of header: title, status, description, image, tags, reward, metadata - unchanged) ... */}
         ...
         <div className="flex flex-wrap gap-2 mb-4">
             ...
             {poll.tags && poll.tags.map((tag, index) => (<span key={index} className="text-xs font-medium bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded-full">#{tag}</span>))}
             {poll.onChain?.hasRewards && (<span className="text-xs font-medium bg-amber-100 text-amber-800 px-2.5 py-1 rounded-full flex items-center"><svg className="w-3 h-3 mr-1 fill-current" viewBox="0 0 20 20"><path d="M10 15a5 5 0 1 0 0-10 5 5 0 0 0 0 10zm0-1.5a3.5 3.5 0 1 1 0-7 3.5 3.5 0 0 1 0 7z M10 0a10 10 0 1 0 0 20A10 10 0 0 0 10 0zm0 18.5a8.5 8.5 0 1 1 0-17 8.5 8.5 0 0 1 0 17z"/></svg>Reward: {formatUSDT(poll.onChain?.rewardPerVoter)} USDT</span>)}
         </div>
         <div className="text-sm text-gray-500 space-y-1">
             ...
             {poll.onChain?.totalVotes !== undefined && (<div className="flex items-center"><span className="font-medium w-20 inline-block flex-shrink-0">Total votes:</span><span>{poll.onChain.totalVotes}</span></div>)}
         </div>
      </div>

      {/* Notification Area */}
      <div className="space-y-4 mb-6">
        {/* ... (rest of notifications: creator, connect, voted, errors, success, reward info - unchanged) ... */}
         {isCreator && poll.onChain?.isActive && (<div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 rounded-r-md"><p className="text-sm"><span className="font-medium">Note:</span> As the poll creator, you cannot vote.</p></div>)}
         {!isConnected && poll.onChain?.isActive && (<div className="p-4 bg-blue-50 border-l-4 border-blue-400 text-blue-700 rounded-r-md"><p className="text-sm font-medium">Connect your wallet or sign in to vote.</p><button onClick={openAuthModal} className="mt-1.5 text-sm text-blue-600 hover:text-blue-800 font-semibold underline">Sign In / Connect Wallet</button></div>)}
         {isConnected && userHasVoted && (<div className="p-4 bg-indigo-50 border-l-4 border-indigo-400 text-indigo-700 rounded-r-md"><p className="text-sm font-medium">You have already cast your vote for this poll.</p>{poll.onChain?.hasRewards && poll.onChain?.hasVotedAndRewarded && (<p className="mt-1 text-sm">Your reward of {formatUSDT(poll.onChain?.rewardPerVoter)} USDT should be reflected in your wallet.</p>)}</div>)}
         {error && (<div className="p-4 bg-red-50 border-l-4 border-red-400 text-red-700 rounded-r-md" role="alert"><p className="text-sm"><span className="font-medium">Error:</span> {error}</p></div>)}
         {pollError && !error && (<div className="p-4 bg-red-50 border-l-4 border-red-400 text-red-700 rounded-r-md" role="alert"><p className="text-sm"><span className="font-medium">Error:</span> {pollError}</p></div>)}
         {voteSuccess && (<div className="p-4 bg-green-50 border-l-4 border-green-400 text-green-700 rounded-r-md" role="alert"><p className="text-sm font-medium">Vote successfully submitted!</p>{rewardReceived && poll.onChain?.hasRewards && (<p className="mt-1 text-sm">Your reward of {formatUSDT(poll.onChain?.rewardPerVoter)} USDT has been sent.</p>)}</div>)}
         {poll.onChain?.hasRewards && poll.onChain?.isActive && !userHasVoted && (<div className="p-4 bg-amber-50 border-l-4 border-amber-400 text-amber-700 rounded-r-md"><h3 className="text-sm font-semibold flex items-center"><svg className="w-4 h-4 mr-1.5 fill-current" viewBox="0 0 20 20"><path d="M10 15a5 5 0 1 0 0-10 5 5 0 0 0 0 10zm0-1.5a3.5 3.5 0 1 1 0-7 3.5 3.5 0 0 1 0 7z M10 0a10 10 0 1 0 0 20A10 10 0 0 0 10 0zm0 18.5a8.5 8.5 0 1 1 0-17 8.5 8.5 0 0 1 0 17z"/></svg>Reward Available!</h3><p className="mt-1 text-sm">Vote on this poll to instantly receive {formatUSDT(poll.onChain?.rewardPerVoter)} USDT in your connected wallet.</p></div>)}
      </div>
              ...
              <div key={index}>
                 ...
                 <div className="flex items-center justify-between">
                   <div className="flex items-center">
                     ...
                     <span className={`text-base font-medium ${isSelected ? 'text-cyan-900' : 'text-gray-800'}`}>{option}</span>
                   </div>
                   {(userHasVoted || !poll.onChain?.isActive) && poll.onChain?.results && (<span className="text-sm font-medium text-gray-500">{poll.onChain.results[index] || 0} votes</span>)}
                 </div>
      ...
      <div className="mt-8 flex justify-start">
        {/* ... (rest of voting button logic - unchanged) ... */}
        {poll.onChain?.isActive && isConnected && !isCreator && !userHasVoted && (
         <button onClick={handleVote} disabled={selectedOption === null || pollLoading || voteSuccess} className={`inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 transition duration-150 ease-in-out ${(selectedOption === null || pollLoading || voteSuccess) ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 active:scale-[0.98]'}`}>
          {pollLoading ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Submitting Vote...</>) : voteSuccess ? ('Vote Submitted!') : ('Submit Vote')}
         </button>)}
        {(!poll.onChain?.isActive || isCreator || userHasVoted) && isConnected && (<p className="text-sm text-gray-500">{ !poll.onChain?.isActive ? "Voting is closed for this poll." : isCreator ? "You cannot vote on your own poll." : userHasVoted ? "You have already voted." : ""}</p>)}
      </div>

      {/* Footer Info */}
      <div className="mt-8 pt-4 border-t border-gray-200">
        {/* ... (rest of footer info - unchanged) ... */}
