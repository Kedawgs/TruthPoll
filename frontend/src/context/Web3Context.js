...
export const Web3Provider = ({ children }) => {
  ...
  const generateUsernameFromAddress = (address) => {
  ...
  // Handle skipping username entry
  const skipUsernameSetup = async () => {
    try {
      ...
      // Save auto-generated username to backend
      const response = await api.post('/users/username', { 
        username: autoUsername,
        address: account,
        isAutoGenerated: true // Flag to indicate this was auto-generated
      });
      ...
      console.error('Error setting auto username:', error);
      return false;
    }
  };
  ...
  const initMagicUser = async (magicInstance) => {
    try {
      ...
      try {
        const response = await api.get(`/users/profile/${userMetadata.publicAddress}`);
  ...
  const initWalletConnection = async () => {
    try {
      ...
      if (accounts.length > 0) {
        ...
        // Check if user has a username
        try {
          const response = await api.get(`/users/profile/${accounts[0]}`);
          if (response.data.success && response.data.data.username) {
            setUserProfile(response.data.data);
            setNeedsUsername(false);
          } else {
